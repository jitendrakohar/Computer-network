...
proc cell_topo {} {
  ...
  $ns suplex-link $nodes(bs2) $nodes(is) 3Mbps 50nodes(ms) DropTail
...
proc stop {} {
       ...
       exec $GETRC-s $did -d $sid -f 0 out.tr| \
        $RAW2SG -a -s 0.01 -m $wrap >> plot.xgr &
       exec xgrap -x time -y packets plot.xgr &
        exit 0
}  
$ns at $stop "stop"
$ns run  
